...
class DetectionValidator(BaseValidator):
    ...
    def __call__(self, trainer=None, model=None):
        ...
        for run in unique_runs:
            ...
            self._set_dataset_attack(self.dataloader.dataset, run)
    ...
    def _override_dataset_load_image(self, dataset) -> None:
        """Override the load_image method of leaf datasets to load pregenerated adversarial images."""
        attack_name = getattr(self, "attack_name", None) or getattr(self.args, "attack_name", None)
        self._set_dataset_attack(dataset, attack_name)

    def _set_dataset_attack(self, dataset, attack_name) -> None:
        """Set the dataset to load clean (raw) or specific adversarial images."""
        # Helper to extract leaf datasets from ConcatDataset/YOLOConcatDataset
        datasets = []
        def get_leaf_datasets(d):
            if hasattr(d, "datasets"):
                for sub_d in d.datasets:
                    get_leaf_datasets(sub_d)
            else:
                datasets.append(d)
        
        get_leaf_datasets(dataset)
        
        for d in datasets:
            if not hasattr(d, "im_files"):
                continue
            
            # Save original load_image if not already saved
            if not hasattr(d, "_orig_load_image"):
                d._orig_load_image = d.load_image
            
            if attack_name == "raw" or not attack_name:
                # Restore original loading
                d.load_image = d._orig_load_image
            else:
                # 1. Validate that all pregenerated images exist (otherwise raise FileNotFoundError)
                for f in d.im_files:
                    path = Path(f)
                    pregen_path = path.parent / f"{path.stem}_{attack_name}.jpg"
                    if not pregen_path.exists():
                        raise FileNotFoundError(
                            f"Pregenerated adversarial image not found at {pregen_path} for attack '{attack_name}'. "
                            f"Please run generate_pregenerated_adv.py first."
                        )
                
                # 2. Bind the custom load_image method
                def custom_load_image(self_dataset, i: int, rect_mode: bool = True):
                    orig_file = self_dataset.im_files[i]
                    path = Path(orig_file)
                    pregen_path = path.parent / f"{path.stem}_{attack_name}.jpg"
                    
                    im = cv2.imread(str(pregen_path))
                    if im is None:
                        raise FileNotFoundError(f"Pregenerated image not found or failed to load: {pregen_path}")
                    
                    h0, w0 = im.shape[:2]  # orig hw
                    if rect_mode:  # resize long side to imgsz while maintaining aspect ratio
                        r = self_dataset.imgsz / max(h0, w0)  # ratio
                        if r != 1:  # if sizes are not equal
                            w, h = (min(math.ceil(w0 * r), self_dataset.imgsz), min(math.ceil(h0 * r), self_dataset.imgsz))
                            im = cv2.resize(im, (w, h), interpolation=cv2.INTER_LINEAR)
                    elif not (h0 == w0 == self_dataset.imgsz):  # resize by stretching image to square imgsz
                        im = cv2.resize(im, (self_dataset.imgsz, self_dataset.imgsz), interpolation=cv2.INTER_LINEAR)
                    if im.ndim == 2:
                        im = im[..., None]
                ...
                import cv2
                import math
                d.load_image = types.MethodType(custom_load_image, d)
            
            # Reset any cached images to ensure they are re-loaded
            d.ims = [None] * len(d.im_files)
            d.im_hw0 = [None] * len(d.im_files)
            d.im_hw = [None] * len(d.im_files)
