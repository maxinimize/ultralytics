...
#         # training: ART format -> YOLO format
#         n, _, H, W = images.shape
#         yolo_tgts = []
...
#         if len(yolo_tgts) == 0:
#             # return a small dummy loss, avoiding calling _loss_fn
...
class YoloV5ForART(nn.Module):
    ...
    def __init__(self, yolo_model: nn.Module, img_size: int):
        ...
        # ensure model in eval mode but allow gradients
        self.model.eval()
    ...
    def forward(self, images: torch.Tensor, targets=None):
        ...
        # use YOLOv8 original loss
        try:
            # save model's original training state
            ...
            with torch.enable_grad():
                # ensure input images require grad
                if not images.requires_grad:
                    images.requires_grad_(True)
                
                # Double check status
                if not torch.is_grad_enabled():
                     LOGGER.warning("torch.is_grad_enabled() IS FALSE inside wrapper! Forcing it.")
                ...
                # set model to train mode to get feature maps (for loss calculation)
